...
st.markdown(
    ...
    h1 {{ font-size: 2.65rem !important; line-height: 3.05rem !important; margin-bottom: 0.35rem !important; }}
    h2 {{ font-size: 2.05rem !important; }}
    h3 {{ font-size: 1.55rem !important; }}
    h4, h5, h6, p, li, label, span {{ color: {WHITE}; font-size: 1.06rem; }}
    .subtitle {{ color: {MUTED}; font-size: 1.25rem; line-height: 1.65rem; }}
    ...
    .small {{ color: {MUTED}; font-size: 1.05rem; line-height: 1.45rem; }}
    .pill {{
...
SCENARIOS = {
    ...
    "Shock permanente de OA/productividad": {
        ...
        "pi_bar": 2.0,
        "pi_e": 2.0,
        "moves": "Cambia la línea de producto potencial vigente. La OA final se reubica porque ahora el pleno empleo es otro, no porque haya ε transitorio.",
    },
...
st.markdown(
    """
    # Simulador NK para el análisis de shocks bajo distintas reglas de política monetaria
    <p class="subtitle"><b>Dinero, Crédito y Bancos · UNS</b><br>Laboratorio guiado para comparar <span class="gold">equilibrio inicial vs. shock</span> bajo <span class="gold">Regla de Taylor</span> y <span class="gold">Regla Monetaria Óptima</span>.</p>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    # Al cambiar de escenario, reiniciamos los shocks del caso.
    # Así un shock de demanda no queda arrastrado cuando se pasa a un shock de oferta.
    if st.session_state.get("_last_scenario") != scenario_name:
        st.session_state["mu_override"] = SCENARIOS[scenario_name]["mu"]
        st.session_state["eps_override"] = SCENARIOS[scenario_name]["eps"]
        st.session_state["dA_override"] = SCENARIOS[scenario_name]["delta_A"]
        st.session_state["dybar_override"] = SCENARIOS[scenario_name]["delta_ybar"]
        st.session_state["_last_scenario"] = scenario_name
    ...
    with st.expander("Mostrar sliders avanzados", expanded=False):
        theta = st.slider("θ · pendiente de OA", 0.20, 1.20, DEFAULTS["theta"], step=0.05, key="theta")
        lam = st.slider("λ · peso del producto en RMO", 0.20, 3.00, DEFAULTS["lam"], step=0.05, key="lam")
        phi = st.slider("φ · sensibilidad de DA a tasa real", 0.40, 2.00, DEFAULTS["phi"], step=0.05, key="phi")
        a = st.slider("a · Taylor responde a inflación", 1.10, 2.40, DEFAULTS["a"], step=0.05, key="a")
        b = st.slider("b · Taylor responde a producto", 0.00, 1.20, DEFAULTS["b"], step=0.05, key="b")
        r_star_base = st.slider("r̄ · tasa real neutral inicial", 0.0, 3.0, DEFAULTS["r_star_base"], step=0.1, key="r_star_base")

        st.markdown("**Tamaño del shock seleccionado**")
        st.caption("Esto es opcional. Para clase, usá primero los escenarios tal como vienen.")
        mu_override = st.slider("μ · DA transitorio", -1.20, 1.20, step=0.1, key="mu_override")
        eps_override = st.slider("ε · OA transitorio", -1.20, 1.20, step=0.1, key="eps_override")
        dA_override = st.slider("∆A · G/A permanente", -1.00, 1.20, step=0.1, key="dA_override")
        dybar_override = st.slider("∆ȳ · potencial permanente", -1.20, 1.00, step=0.1, key="dybar_override")

    # Si no se abrió el expander, tomamos igualmente los valores de session_state/preset.
    theta = st.session_state.get("theta", DEFAULTS["theta"])
    lam = st.session_state.get("lam", DEFAULTS["lam"])
    phi = st.session_state.get("phi", DEFAULTS["phi"])
    a = st.session_state.get("a", DEFAULTS["a"])
    b = st.session_state.get("b", DEFAULTS["b"])
    r_star_base = st.session_state.get("r_star_base", DEFAULTS["r_star_base"])
    mu_override = st.session_state.get("mu_override", SCENARIOS[scenario_name]["mu"])
    eps_override = st.session_state.get("eps_override", SCENARIOS[scenario_name]["eps"])
    dA_override = st.session_state.get("dA_override", SCENARIOS[scenario_name]["delta_A"])
    dybar_override = st.session_state.get("dybar_override", SCENARIOS[scenario_name]["delta_ybar"])
...
with intro_right:
    st.markdown(
        ...
        <p class="small">Compará siempre las curvas punteadas iniciales con las curvas sólidas finales.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with left:
    ...
    def build_figure(alpha=1.0):
        # alpha=0 muestra el equilibrio inicial; alpha=1 muestra el shock completo.
        oa_anim = initial["oa"] * (1 - alpha) + final["oa"] * alpha
        rpm_anim = initial["rpm"] * (1 - alpha) + final["rpm"] * alpha
        y_anim = initial["y_eq"] * (1 - alpha) + final["y_eq"] * alpha
        pi_anim = initial["pi_eq"] * (1 - alpha) + final["pi_eq"] * alpha
        ybar_anim = delta_ybar * alpha
        pibar_anim = pi_bar_0 * (1 - alpha) + pi_bar * alpha

        fig = go.Figure()

        # Curvas iniciales punteadas / de referencia
        fig.add_trace(go.Scatter(
            x=y_grid, y=initial["oa"], mode="lines", name="OA₀ inicial",
            line=dict(width=4, color="rgba(210,230,255,0.55)", dash="dot")
        ))
        fig.add_trace(go.Scatter(
            x=y_grid, y=initial["rpm"], mode="lines", name="RPM₀ inicial",
            line=dict(width=4, color="rgba(245,220,120,0.60)", dash="dot")
        ))

        # Curvas finales/en movimiento
        fig.add_trace(go.Scatter(
            x=y_grid, y=oa_anim, mode="lines", name="OA₁ con shock",
            line=dict(width=6, color=BLUE)
        ))
        fig.add_trace(go.Scatter(
            x=y_grid, y=rpm_anim, mode="lines", name=final["rpm_label"].replace("final", "₁"),
            line=dict(width=6, color=GOLD_LIGHT, dash="dash")
        ))

        # Equilibrios
        fig.add_trace(go.Scatter(
            x=[initial["y_eq"]], y=[initial["pi_eq"]], mode="markers+text", name="E₀ inicial",
            marker=dict(size=16, color=GOLD, line=dict(width=3, color="#FFFFFF")),
            text=["E₀"], textposition="bottom center",
            textfont=dict(size=20, color="#FFFFFF")
        ))
        fig.add_trace(go.Scatter(
            x=[y_anim], y=[pi_anim], mode="markers+text", name="E₁ con shock",
            marker=dict(size=20, color=PINK, line=dict(width=3, color="#FFFFFF")),
            text=["E₁"], textposition="top center",
            textfont=dict(size=20, color="#FFFFFF")
        ))

        # Referencias
        fig.add_vline(x=0, line_width=1, line_dash="dot", line_color="rgba(255,255,255,0.45)", annotation_text="ȳ₀", annotation_font_color="white", annotation_font_size=16)
        if abs(delta_ybar) > 0.01:
            fig.add_vline(x=ybar_anim, line_width=2, line_dash="dot", line_color="rgba(255,91,110,0.95)", annotation_text="ȳ₁", annotation_font_color="white", annotation_font_size=16)
        fig.add_hline(y=pi_bar_0, line_width=1, line_dash="dot", line_color="rgba(255,255,255,0.42)", annotation_text="π̄₀", annotation_font_color="white", annotation_font_size=16)
        if abs(pi_bar - pi_bar_0) > 0.01:
            fig.add_hline(y=pibar_anim, line_width=2, line_dash="dot", line_color="rgba(255,91,110,0.90)", annotation_text="π̄₁", annotation_font_color="white", annotation_font_size=16)
        ...
        fig.update_layout(
            ...
            legend=dict(
                ...
                bgcolor="rgba(16,26,47,0.88)",
                bordercolor="rgba(201,168,76,0.45)",
                borderwidth=1,
            ),
            margin=dict(l=70, r=30, t=30, b=130),
            xaxis=dict(
                title=dict(text="Producto: y − ȳ₀", font=dict(size=19, color="#FFFFFF")),
            ...
            yaxis=dict(
                title=dict(text="Inflación π", font=dict(size=19, color="#FFFFFF")),
                tickfont=dict(size=16, color="#FFFFFF"),
                zeroline=False,
                gridcolor="rgba(255,255,255,0.16)",
                range=[-1, 6],
    ...
    chart_box = st.empty()
    animate = st.button("▶ Ver desplazamiento paso a paso", help="Muestra lentamente el paso de E₀ al escenario con shock.")
    if animate:
        import time
        for alpha in np.linspace(0, 1, 9):
            chart_box.plotly_chart(build_figure(float(alpha)), use_container_width=True, config={"displayModeBar": False})
            time.sleep(0.22)
    else:
        chart_box.plotly_chart(build_figure(1.0), use_container_width=True, config={"displayModeBar": False})
